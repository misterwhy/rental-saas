        ...
        <div class="action">
            @auth
                @if(auth()->user()->isLandlord())
                    <a href="{{ route('properties.create') }}" class="save-btn">Add Property</a>
                @endif
            @else
                <a href="{{ route('register') }}" class="save-btn">Register</a>
            @endauth
        </div>
    </div>
    ...
    <div class="room-grid">
        @if(isset($featuredProperties) && $featuredProperties->count() > 0)
            @foreach($featuredProperties as $property)
                <div class="card111">
                    <div class="header111">
                        <div class="header-text111">{{ $property->property_type }}</div>
                    </div>
                    ...
                    <div class="details111">
                        ...
                        <div class="user-info111">
                            <div class="user-name111">
                                ...
                                <div class="user-name1">{{ $property->landlord->name }}</div>
                            </div>
                            <div class="status111">
                    ...
                    <div class="editing-buttons">
                        <a href="{{ route('properties.show', $property) }}" class="editing-buttons1">View Details</a>
                    </div>
                </div>
            @endforeach
        @else
